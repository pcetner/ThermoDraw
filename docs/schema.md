...
| `angle` | which way the label is pushed off the node |

`fixed` draws the boundary wall and connects to it. `break` stops short of the
...
`kind` is `diss`, `radin`, `flow` or `flux`. `at` and `angle` place the arrow;
by default it comes in horizontally from the left and its head lands on the
node.
