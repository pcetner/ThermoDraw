...
ENTRIES = {
    ...
    "stream": dict(
        where="node",
        what="The one node that is not a place. A medium moving through the "
             "drawing arrives at one temperature and leaves at another, and "
             "the difference is the heat it carried away. It states "
             "<code>inlet</code> and <code>outlet</code> instead of "
             "<code>value</code>, because it is not at one temperature, and "
             "<code>rate</code> is what the rise came to. The streamlines "
             "beneath it are the same mark a convection box is filled with, "
             "and they say the medium is going without saying which way. A "
             "resistance joined to one needs <code>reference</code> to say "
             "which of the two temperatures it works from.",
        use="Steel strip through a reheat furnace. It goes in cold and comes "
            "out hot, and what the furnace has to supply is exactly the "
            "difference.",
        code={"id": "strip", "kind": "stream", "label": "Steel strip",
